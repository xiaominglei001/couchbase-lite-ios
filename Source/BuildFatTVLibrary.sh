...
if [ ${PLATFORM_NAME} = "appletvsimulator" ]
then
...
echo "XCode has selected SDK: ${PLATFORM_NAME} with version: ${SDK_VERSION} (although back-targetting: ${APPLETVOS_DEPLOYMENT_TARGET})"
...
CURRENTCONFIG_DEVICE_DIR=${SYMROOT}/${CONFIGURATION}-appletvos
CURRENTCONFIG_SIMULATOR_DIR=${SYMROOT}/${CONFIGURATION}-appletvsimulator

echo "Taking device build from: ${CURRENTCONFIG_DEVICE_DIR}"
echo "Taking simulator build from: ${CURRENTCONFIG_SIMULATOR_DIR}"

CREATING_UNIVERSAL_DIR=${SYMROOT}/${CONFIGURATION}-${PRODUCT_NAME}-appletvos-universal
